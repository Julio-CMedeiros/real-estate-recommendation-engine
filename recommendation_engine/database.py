...
# Guards the check-then-seed sequence below. FastAPI serves each request on a
# threadpool worker, so concurrent first requests against a brand-new
# (nonexistent) DB file can otherwise race: `path.exists()` can flip to True
# because *another* thread's `sqlite3.connect()` call already created the
# (still-empty) file, causing this thread to skip seeding entirely and hand
# back a connection with no tables. The lock makes "check, connect, seed" one
# atomic step per process. `_seed()` is additionally written to be idempotent
# (CREATE TABLE IF NOT EXISTS / INSERT OR IGNORE) as defense in depth.
...
def _seed(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        INSERT OR IGNORE INTO neighborhoods VALUES
            (1, 'Chiado', 'Lisbon'),
            (2, 'Alfama', 'Lisbon'),
        ...
        INSERT OR IGNORE INTO properties (id, neighborhood_id, title, type, price, area_m2, bedrooms, bathrooms, floor, has_parking, has_terrace, energy_rating, listed_date, status) VALUES
        ...
        INSERT OR IGNORE INTO price_history (property_id, price, changed_date, event) VALUES
        ...
        INSERT OR IGNORE INTO market_snapshots (neighborhood_id, month, avg_price_m2, listings_count, avg_days_on_market, sold_count) VALUES
